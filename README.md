...
Получившиеся секции содержат изменения из каждой ветки:
...
#### Разрешение конфликтов с помощью VS Code

В окне редактора появятся маркеры конфликта.

![Маркеры конфликта в VS Code](https://pictures.s3.yandex.net/resources/M4_T4_04_1689626826.png)
...
Вы увидите окно разрешения конфликтов. Оно состоит из трёх частей:
1. в **левой** части содержатся **новые изменения**
2. в **правой** — **текущие**,
3. в **нижней** — **результат**.
...
Когда все конфликты разрешены, можно нажать на **Complete Merge** (англ. «завершить слияние»).

Остался последний шаг — **создать коммит слияния**. VS Code попытается сделать это автоматически в левой части экрана. Измените сообщение коммита, если нужно, и нажмите на кнопку **Commit**. Этот шаг также можно выполнить через консоль.
...
Ваши изменения и текущие изменения в `main` получится «смёржить» автоматически, потому что GitHub сможет самостоятельно разрешить конфликт. В этом случае кнопка **Merge pull request** в интерфейсе пул-реквеста всё ещё будет зелёной.

##### Второй вариант

Два изменения не могут существовать вместе, и вы увидите следующее предупреждение.

![Предпреждение GitHub о конфликте в ветках](https://pictures.s3.yandex.net/resources/M4_T4_04-2_1689627475.png)
...
`git merge <название ветки>` - выполнить слияние ветки `<название ветки>` и текущей ветки (изменения вольются в текущую ветку)
