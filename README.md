...
> [!NOTE]
> В любой момент можно добавить дополнительны коммиты в ветку - они автоматически попадут в открытый пул-реквест после пуша.
...
`git diff HEAD~ HEAD` - сравнить предыдущий коммит и последний (испльзуется суффикс навигации)
...
`git branch -D <название ветки>` - удалить ветку `<название ветки>` (более безопасный вариант с флагом `-d` - он удалит ветку только, если она полностью была объединена с другой)
